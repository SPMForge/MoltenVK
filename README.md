...
- `Package.swift` is generated by `Scripts/SwiftPackage/render_package_manifest.py` and resolves the local `Artifacts/MoltenVK.xcframework` when it exists
- otherwise it falls back to a release asset at `releases/download/MoltenVK-v<version>/MoltenVK.xcframework.zip`
- the package product exports a `MoltenVK` Clang target that exposes the public headers and depends on the `MoltenVKBinary` XCFramework target for linking
- `Scripts/SwiftPackage/build_swift_package.sh` builds archived dynamic frameworks with `xcodebuild archive`, assembles a mergeable `MoltenVK.xcframework`, validates the slices, and also publishes the static/header companion assets
- the current binary package contract is aligned with the published release assets: `macOS`, `iOS`, and `iOS Simulator`
...
- `.github/workflows/moltenvk-spm-release.yml` runs `semantic-release` on `main` for automated alpha prereleases and also exposes a manual stable publish path that requires the target version plus explicit stable intent
...
- stable releases are intentionally manual-only and must be published from `main` with an explicit version and confirmation step
