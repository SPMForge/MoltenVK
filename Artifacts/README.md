...
The dynamic SwiftPM artifact is assembled from archived `macOS`, `iOS`, and `iOS Simulator` framework slices. Those framework slices carry the public headers and `Modules/module.modulemap` internally, use framework-style `<MoltenVK/...>` imports for same-framework public dependencies, and the final zipped XCFramework is validated for mergeable metadata plus import surface before publication.
